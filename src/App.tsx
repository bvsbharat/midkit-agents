...
const AGENTS = [
  { id: 'comm', name: 'Communication Agent', role: 'Drafts & Follow-ups', icon: MessageSquare, color: 'bg-green-100 text-green-600', rating: '5.0', desc: 'Monitors the conversation to automatically draft contextual follow-up emails, meeting summaries, and internal notes.' },
  { id: 'scheduler', name: 'Scheduler Agent', role: 'Calendar Management', icon: Calendar, color: 'bg-amber-100 text-amber-600', rating: '4.8', desc: 'Identifies scheduling intents and automatically proposes meeting times based on availability of all required participants.' },
  { id: 'analytics', name: 'Analytics Agent', role: 'Data & CRM', icon: LineChart, color: 'bg-purple-100 text-purple-600', rating: '4.9', desc: 'Pulls historical sales data, assesses deal health, and automatically prepares CRM updates based on call outcomes.' },
];
...
const TabBar = ({ active, setActive }: { active: string, setActive: (tab: string) => void }) => {
  const tabs = ['Live Copilot', ...AGENTS.map(a => a.name), 'Summary'];
  ...
  return (
    <div className="flex gap-2 mb-6 overflow-x-auto pb-2 custom-scrollbar">
      {tabs.map(tab => (
        ...
        >
          {tab !== 'Live Copilot' && tab !== 'Summary' && <Bot size={16} className={active === tab ? 'text-gray-900' : 'text-gray-400'} />}
          {tab === 'Live Copilot' && <BrainCircuit size={16} className={active === tab ? 'text-gray-900' : 'text-gray-400'} />}
          {tab}
        </button>
      ))}
    </div>
...
const ActionCard = ({ action, handleAction }: { action: any, handleAction: (id: number, approved: boolean) => void }) => (
  <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4 transition-all hover:shadow-md hover:border-gray-200">
...
const AgentView = ({ agent, actions, handleAction }: { agent: any, actions: any[], handleAction: (id: number, approved: boolean) => void }) => {
  const agentActions = actions.filter(a => a.agent === agent.name);

  return (
    <div className="grid grid-cols-12 gap-6 flex-1 min-h-0 animate-in fade-in duration-300">
      ...
      <div className="col-span-12 lg:col-span-4 flex flex-col gap-6 min-h-0">
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 flex flex-col">
          ...
          <p className="text-sm font-medium text-gray-500 mb-4">{agent.role}</p>
          <p className="text-sm text-gray-600 leading-relaxed mb-6">{agent.desc}</p>
          ...
          <div className="grid grid-cols-2 gap-4 pt-6 border-t border-gray-100 mt-auto">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="text-2xl font-bold text-gray-900 mb-1 flex items-center gap-1">
                {agent.rating} <Star size={16} className="fill-yellow-400 text-yellow-400" />
              </div>
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-sm text-gray-800">Analyzed transcript for intent</p>
            ...
            <div className="flex gap-3">
              <div className="w-2 h-2 rounded-full bg-gray-300 mt-1.5 shrink-0"></div>
              <div>
                <p className="text-sm text-gray-800">Generated action item</p>
            ...
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-sm text-gray-800">Action approved by Erlina</p>
      ...
      {/* Right Column: Agent's Pending Actions */}
          ...
          <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2">
            {agentActions.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center text-gray-400">
                ...
                <p className="text-xs">No pending actions for this agent right now.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {agentActions.map(action => (
                  <ActionCard key={action.id} action={action} handleAction={handleAction} />
                ))}
              </div>
            )}
          </div>
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState('Live Copilot');
  ...
  return (
    <div className="flex h-screen bg-[#F4F5F7] font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col p-8 overflow-hidden">
        ...
        {activeTab === 'Live Copilot' && (
          <LiveDashboard actions={actions} handleAction={handleAction} />
        )}

        {activeAgent && (
          <AgentView agent={activeAgent} actions={actions} handleAction={handleAction} />
        )}

        {/* Placeholders for other tabs */}
        {activeTab === 'Summary' && (
          <div className="flex-1 flex items-center justify-center bg-white rounded-[24px] border border-gray-100 shadow-sm animate-in fade-in duration-300">
            <div className="text-center">
              <FileText size={48} className="mx-auto text-gray-300 mb-4" />
              <h2 className="text-xl font-semibold text-gray-900 mb-2">{activeTab}</h2>
              <p className="text-gray-500">Meeting summary will be generated after the call ends.</p>
            </div>
          </div>
        )}
      </div>
